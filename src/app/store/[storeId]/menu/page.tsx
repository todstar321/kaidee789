...
export default function MenuManagementPage({ params }: { params: { storeId: string } }) {
  ...
  const fetchMenuData = async () => {
    try {
      const [resStore, resMenu] = await Promise.all([
        ...
        fetch(`/api/menu?store_id=${params.storeId}&_t=${Date.now()}`).then(r => r.json()),
      ]);
  ...
  const handleCreateCategory = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        setCatName('');
        setCatFeedback('✓ เพิ่มหมวดหมู่สำเร็จแล้ว!');
        await fetchMenuData();
        if (data.id) {
          setItemForm(prev => ({ ...prev, category_id: data.id }));
        }
      } else {
        alert(data.error || 'เกิดข้อผิดพลาดในการเพิ่มหมวดหมู่');
      }
    } catch (err) {
      console.error(err);
  ...
  const handleUpdateCategory = async (catId: string, newName: string) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert(data.error || 'แก้ไขหมวดหมู่ไม่สำเร็จ');
      }
    } catch (err) {
      console.error(err);
    }
  ...
  const handleDeleteCategory = async (catId: string, name: string) => {
    const itemsInCat = menuItems.filter(m => m.category_id === catId).length;
    if (itemsInCat > 0) {
      if (!confirm(`หมวดหมู่ "${name}" มีเมนูอาหารอยู่ ${itemsInCat} รายการ หากลบหมวดนี้ เมนูจะไม่แสดงในหมวดหมู่ คุณต้องการลบใช่หรือไม่?`)) {
        return;
      }
    } else {
    ...
    try {
      const res = await fetch(`/api/menu?action=category&category_id=${catId}&store_id=${params.storeId}`, {
        method: 'DELETE',
      });
      const data = await res.json();
      if (data.success) {
          ...
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-200 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100 mb-4">
              <div>
                ...
                <p className="text-[11px] text-slate-400">เพิ่ม แก้ไข หรือลบหมวดหมู่สำหรับจัดกลุ่มเมนูอาหาร</p>
              </div>
              <button
                onClick={() => {
            ...
            <form onSubmit={handleCreateCategory} className="space-y-3 mb-6 bg-slate-50 p-4 rounded-xl border border-slate-200/80">
              ...
              <div className="flex gap-2">
                <input
                  type="text"
                  required
                  placeholder="พิมพ์ชื่อหมวดหมู่ เช่น อาหารจานเดียว, เครื่องดื่ม..."
                  value={catName}
                  onChange={(e) => setCatName(e.target.value)}
                  className="flex-1 px-3 py-2 rounded-xl bg-white border border-slate-200 text-slate-900 text-xs focus:outline-none focus:border-purple-500 shadow-sm"
                />
                <button
                  type="submit"
                  disabled={isSavingCat || !catName.trim()}
                  className="px-4 py-2 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-bold text-xs shadow-sm transition disabled:opacity-50 flex items-center gap-1.5"
                >
                  <Plus className="w-3.5 h-3.5" />
                  <span>{isSavingCat ? 'กำลังเพิ่ม...' : 'เพิ่มหมวด'}</span>
                </button>
              </div>

              {/* Quick Suggestion Chips */}
              <div>
                <span className="text-[10px] text-slate-400 block mb-1.5 font-medium">กดเลือกชื่อหมวดหมู่ยอดนิยม:</span>
                <div className="flex flex-wrap gap-1.5">
                  {[
                    ...
                    <button
                      key={sug}
                      type="button"
                      onClick={() => setCatName(sug)}
                      className="px-2 py-0.5 rounded-lg bg-white hover:bg-purple-50 hover:text-purple-700 hover:border-purple-200 text-slate-600 border border-slate-200 text-[11px] font-medium transition"
                    >
                      {sug}
                    </button>
                  ))}
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-bold text-slate-700">หมวดหมู่ทั้งหมดในร้าน ({categories.length})</span>
              </div>

              {categories.length === 0 ? (
                <div className="p-4 text-center text-slate-400 text-xs bg-slate-50 rounded-xl border border-slate-200">
                  ยังไม่มีหมวดหมู่อาหาร กรุณาเพิ่มหมวดหมู่แรกด้านบน
                </div>
              ) : (
                        ...
                        {isEditing ? (
                          <div className="flex items-center gap-2 flex-1">
                            <input
                              ...
                              className="flex-1 px-2.5 py-1 rounded-lg bg-white border border-purple-300 text-slate-900 text-xs focus:outline-none"
                              autoFocus
                            />
                            <button
                              type="button"
                            ...
                            <button
                              ...
                              className="px-2 py-1 rounded-lg bg-slate-200 text-slate-600 text-xs"
                            >
                              ยกเลิก
                            </button>
                          </div>
                          ...
                          <>
                            <div className="flex items-center gap-2">
                              ...
                              <span className="px-2 py-0.2 rounded-full bg-slate-200 text-slate-600 text-[10px] font-semibold">
                                {count} เมนู
                              </span>
                            </div>
